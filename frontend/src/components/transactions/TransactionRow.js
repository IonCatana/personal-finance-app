...
import { getRecurringBillStatus } from "@utils/recurringBills";
...
const TransactionRow = ({
  ...
  const recurringBillStatus = transaction.recurring
    ? getRecurringBillStatus(transaction, today)
    : null;
  const isPast = transaction.recurring
    ? recurringBillStatus.isPaid
    : isBefore(transactionDate, today);

  let textColor = theme.palette.grey[900];
  let icon = null;

  if (transaction.recurring) {
    if (recurringBillStatus.isPaid) {
      textColor = theme.palette.secondaryColors.green;
      icon = CHECK_ICON;
    } else if (recurringBillStatus.isDueSoon) {
      textColor = theme.palette.secondaryColors.red;
      icon = WARNING_ICON;
    }
  } else if (isPast) {
